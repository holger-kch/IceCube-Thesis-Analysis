...
**Why this stage exists:** Build or locate the mixed MC/data pulse-level tables used by later
stages.

**How the stage is produced:**
...
**How it feeds the rest of the analysis:** this stage produces the derived
databases/parquets used by training and plotting. These are intentionally not
tracked.

## Stage 1: Split Stopped And Through-Going Muons

**Why this stage exists:** Train on MC truth labels, then apply the same classifier to MC and
...
**Why this stage exists:** Quantify how separable real burnsample muons and simulated Muon Gun
...
**Why this stage exists:** Inspect which pulse-level and event-level variables differ before
corrections.
...
**Why this stage exists:** Test whether part of the mismatch comes from different reconstructed
...
**Why this stage exists:** Merge sub-`0.3 PE` HLC satellite pulses into nearby above-threshold
...
**Why this stage exists:** Ask the trained MC-vs-data classifier which pulse features carry
...
**Why this stage exists:** Train HLC/SLC models on data, apply them to MC, and flip the most
...
**Why this stage exists:** Search for afterpulse-like structure in the corrected charge-time
plane.
...
**Why this stage exists:** Train a direction model that also predicts a vMF concentration
...
- low/high-`kappa` MC split summary generated by [make_mc_kappa_split_table.py](../analysis/MC_vs_BS_analysis/GBreweighting/validation/make_mc_kappa_split_table.py)

## Stage 10: Final ROC And Logit Comparison

**Why this stage exists:** Collect staged MC-vs-data scores and produce the final result
figures.
