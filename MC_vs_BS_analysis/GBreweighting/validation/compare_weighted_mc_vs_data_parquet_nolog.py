#!/usr/bin/env python3
"""Parquet-backed MC-vs-data comparison — linear-axis (zoomed) variant.

Sister script to compare_weighted_mc_vs_data_parquet.py. Same input and
caching logic, but:

  * All y-axes rendered linearly (no log).
  * Per-class bin ranges hardcoded so the bulk of each distribution
    fills the panel instead of being squeezed against zero. See
    NOLOG_AGG_XLIM / NOLOG_PULSE_XLIM below for the exact zoom ranges.

Cache namespace is separate (`parquet_nolog` tag) so this script and
the log variant do not invalidate each other.
...
def main() -> None:
    ...
    parser.add_argument("--out",
                        default="mc_vs_data_combined_parquet_nolog.png",
