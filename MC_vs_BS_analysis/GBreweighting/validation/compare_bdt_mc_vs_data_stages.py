#!/usr/bin/env python3
"""BDT MC-vs-data benchmark across 3 processing stages.
...
BDT over 3 stages of pulse processing:

    stage1: non-merged          (SplitInIcePulses, no dtype fix)
    stage2: + float fix         (SplitInIcePulses, rde/pmt_area cast to float32)
    stage3: + merged            (SplitInIcePulses_merged, with dtype fix)

For each (class, stage) pair = 6 BDT trainings. Output:
    validation/plots/bdt_stages_roc_stopped.png   (3 ROC curves overlaid)
    validation/plots/bdt_stages_roc_through.png   (3 ROC curves overlaid)
...
CPU-only, ~5 min per (class, stage) ≈ ~30 min total. No GPU.
"""
...
def run_stage(class_name: str, stage: dict, rebuild: bool = False) -> dict:
    ...
    if (metrics_file.exists() and roc_file.exists()
            and pred_file.exists() and not rebuild):
...
def plot_roc_overlay(class_name: str, stage_results: list[dict],
    ...
    # red → orange → green: signals "improvement toward diagonal"
    colors = ["#d62728", "#ff7f0e", "#2ca02c"]
    for i, (stg, color) in enumerate(zip(stage_results, colors)):
        # Highlight the baseline (non-merged) curve with thicker line
