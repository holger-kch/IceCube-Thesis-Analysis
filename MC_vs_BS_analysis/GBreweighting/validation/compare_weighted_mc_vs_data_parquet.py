#!/usr/bin/env python3
"""Parquet-backed MC-vs-data comparison — identical output to
compare_weighted_mc_vs_data.py but reads the pre-exported parquet tables
in validation/data_parquet/ instead of streaming SQLite.

End-to-end `--rebuild` drops from ~3-4 h to ~5-15 min.

Prerequisites:
    python export_to_parquet.py   (one-off, ~2-3 h)
  → validation/data_parquet/{mc,data}_{table}_{class}.parquet
    where table ∈ {SplitInIcePulses, SplitInIcePulses_merged},
    class ∈ {stopped, through}.

Notes vs. the SQLite version:
  * No per-chunk streaming needed — each file is loaded once per pass
    (needed columns only, pyarrow dictionary + zstd keeps it compact).
  * n_doms is computed via pandas unique-tuple groupby instead of
    SQL `COUNT(DISTINCT dom_x||','||dom_y||','||dom_z)`.
  * The per-event pulse-subsample cache is dropped: users can read the
    full parquet files directly for any 2D / re-binning work.
    Aggregate DataFrames (agg_mc_df / agg_dt_df) are still cached.
"""
...
# Parquet-based cache uses the "_parquet" suffix to stay out of the
# original script's cache namespace.
_CACHE_TAG = "parquet"
...
PULSE_HIST_SPECS = {
    "dom_time": {"nbins": 60, "xlabel": "dom_time [ns]",
                 "xlim": (0, 30000)},
    "charge":   {"nbins": 60, "logy": True, "xlabel": "charge [PE]"},
    "dom_x":    {"nbins": 60, "xlabel": "dom_x [m]"},
    "dom_y":    {"nbins": 60, "xlabel": "dom_y [m]"},
    "dom_z":    {"nbins": 60, "xlabel": "dom_z [m]"},
    "width":    {"nbins": 60, "logy": True, "xlabel": "pulse width [ns]",
    ...
    "is_errata_dom": {"categorical": True, "logy": True,
                      "xlabel": "is_errata_dom flag",
                      "title": "is_errata_dom\nDOMs with known hardware issues"},
}

AGG_HIST_SPECS = {
    "n_hits":    {"nbins": 60, "logy": True, "xlabel": "N_hits per event"},
    "n_doms":    {"nbins": 60, "logy": True, "xlabel": "N_doms per event"},
    "qtot":      {"nbins": 60, "logy": True, "xlabel": "Q_tot [PE]"},
    "qmax":      {"nbins": 60, "logy": True, "xlabel": "Q_max [PE]"},
    "t_std":     {"nbins": 60, "logy": True,
                  "xlabel": "charge-weighted t_std [ns]"},
    "t_extent":  {"nbins": 60, "logy": True,
                  "xlabel": "t_max − t_min [ns]"},
...
# ---------------------------------------------------------------------------
def make_pulse_bins(cls: str, pad: float = 0.05) -> dict:
    ...
    scan_vars = [v for v in cont_vars if PULSE_HIST_SPECS[v].get("xlim") is None]
    fixed_vars = [v for v in cont_vars if PULSE_HIST_SPECS[v].get("xlim") is not None]
...
def make_agg_bins(agg_mc: pd.DataFrame, agg_data: pd.DataFrame,
                  pad: float = 0.05) -> dict[str, np.ndarray]:
    edges = {}
...
def process_class(class_name: str, rebuild: bool = False) -> dict:
    print(f"\n{'='*60}\n  Class: {class_name}\n{'='*60}", flush=True)
    ...
    pulse_edges = make_pulse_bins(class_name)
    ...
    agg_edges = make_agg_bins(agg_mc, agg_dt)
...
def plot_side_by_side(stopped: dict, through: dict, out_path: Path) -> None:
    ...
    footer = (
        ...
        f"GB reweighting fitted on (zenith_pred, azimuth_pred) per class;\n"
...
def main() -> None:
    ...
    parser.add_argument("--out",
                        default="mc_vs_data_combined_parquet.png",
