#!/usr/bin/env python3
"""DynEdge-inspired Transformer for muon reconstruction — v2.

Changes from v1:
  - Energy: trained in LINEAR GeV space with LogCoshLoss + softplus output
    (matching GraphNeT's EnergyReconstruction approach)
  - Position_z: trained in LINEAR metre space with LogCoshLoss (no normalization)
  - Scheduler: ReduceLROnPlateau instead of OneCycleLR
  - Longer patience and more epochs allowed

Targets: direction | energy | position_z  (one at a time)

Usage:
    python train_dynedge_transformer_v2.py --target energy
    python train_dynedge_transformer_v2.py --target position_z
    python train_dynedge_transformer_v2.py --target direction
"""
...
# Energy/position_z normalization scales (for loss weighting only)
ENERGY_SCALE = 500.0   # approximate mean energy in GeV
POS_Z_SCALE = 330.0    # approximate std of position_z in metres
...
TARGET_CONFIGS = {
    ...
    "energy": {
        ...
        "process": lambda row: np.array(
            [np.float32(row[0])], dtype=np.float32  # raw GeV, no log10
        ),
    },
    "position_z": {
        "columns": ["position_z"],
        "process": lambda row: np.array(
            [np.float32(row[0])], dtype=np.float32  # raw metres, no normalization
        ),
    },
}
...
class ScalarHead(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_softplus:
            # Match GraphNeT EnergyReconstruction: softplus ensures positive output
            out = F.softplus(out, beta=0.05) + 1e-6
        return out
    ...
    def __init__(self, input_dim=INPUT_DIM, d_model=256, num_layers=6,
        ...
        if target == "direction":
            ...
            self.head = ScalarHead(
                d_model, head_hidden_dim, dropout,
                use_softplus=(target == "energy"),
            )
...
def log_cosh_loss(pred, target):
    """LogCosh loss — same as GraphNeT's LogCoshLoss.

    Behaves like MSE for small errors and like MAE for large errors,
    with a smooth transition around |error| ~ 1.
    """
    diff = pred - target
...
def validate_scalar(model, loader, device, use_amp, target_name):
    ...
    # Val loss = LogCosh in scaled space (same as training)
    if target_name == "energy":
        ...
        scale = 1.0
    val_loss = log_cosh_loss(all_preds / scale, all_targets / scale).item()

    # MAE in original units (already in linear space)
    errors = (all_preds - all_targets).abs().numpy()
...
def main():
    ...
    if is_direction:
        ...
        # LogCosh in linear GeV space, scaled to keep gradients reasonable
        def loss_fn(pred, targets):
            return log_cosh_loss(
                pred.squeeze(-1) / ENERGY_SCALE,
                targets.squeeze(-1) / ENERGY_SCALE,
            )
    else:  # position_z
        # LogCosh in linear metre space, scaled
        def loss_fn(pred, targets):
            return log_cosh_loss(
                pred.squeeze(-1) / POS_Z_SCALE,
                targets.squeeze(-1) / POS_Z_SCALE,
            )
    ...
    loss_name = 'AngularDistance' if is_direction else 'LogCosh (linear space)'
    ...
    if is_direction:
        ...
        results_df = pd.DataFrame({
            "event_no": event_ids,
            f"{args.target}_true": targets_flat,
            f"{args.target}_pred": preds_flat,
            f"{args.target}_error": np.abs(preds_flat - targets_flat),
        })
