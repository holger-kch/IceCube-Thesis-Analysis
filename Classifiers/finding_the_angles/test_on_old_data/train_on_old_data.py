#!/usr/bin/env python3
"""DynEdge direction reconstruction trained on old MuonGun data.

Same architecture and training setup as train_3.py, but runs on:
  /groups/icecube/holgerkc/Thesis_Analysis/Classifiers/finding_the_angles/test_on_old_data/merged/events.db

Outputs (PREFIX = 'old_data_direction'):
  - old_data_direction_results.csv
  - old_data_direction_metrics.json
  - old_data_direction_state_dict.pth
  - old_data_direction_model_config.yml
  - old_data_direction_split.json
  - old_data_direction_train_config.json
"""
...
# ── Configuration ────────────────────────────────────────────────
DB_PATH = "/groups/icecube/holgerkc/Thesis_Analysis/Classifiers/finding_the_angles/test_on_old_data/merged/events.db"
...
OUTPUT_DIR = Path("/groups/icecube/holgerkc/Thesis_Analysis/Classifiers/finding_the_angles/test_on_old_data")
PREFIX = "old_data_direction"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train DynEdge for direction on old MuonGun data.")
...
def main() -> None:
    ...
    with sqlite3.connect(DB_PATH) as con:
        event_ids = pd.read_sql_query(
            f"SELECT event_no FROM truth LIMIT {n_fetch}", con,
