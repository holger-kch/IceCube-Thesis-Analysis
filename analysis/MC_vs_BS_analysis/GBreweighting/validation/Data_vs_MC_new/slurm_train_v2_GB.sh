...
#SBATCH --job-name=data_vs_mc_new_GB
#SBATCH --array=0-1%2
#SBATCH --gres=gpu:1
...
OUT_DIR="${SCRIPT_DIR}/results/transformer_data_vs_mc_${CLASS_NAME}_hlc_rde_v2_GB"
...
echo "=== Data-vs-MC transformer GB: ${CLASS_NAME}, v2 parquets, HLC/RDE, final_weight ==="
echo "Node: $(hostname)"
...
"${PYTHON}" "${SCRIPT_DIR}/train_mcdata_parquet.py" \
    ...
    --parquet-suffix v2 \
    --pulse-file-template "{source}_SplitInIcePulses_{cls}_{parquet_suffix}.parquet" \
