...
12 files = 2 sources × 2 pulsemaps × 2 classes (pulses)
+ 1 source × 2 classes (truth) + 2 class-specific weight CSVs.
...
| `GB_and_base_weights_{stopped,through}.csv` | Per-event MC/data weights used by the GB reweighting and validation training | per-event |
...
Per-event weights are not stored in the parquet pulse/truth files. They are
included alongside them in this directory:
