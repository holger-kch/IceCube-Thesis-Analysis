#!/usr/bin/env python3
"""Sister to fit_GBreweighter.py — adds log10(qtot) as a 3rd feature.

Diagnostic: the original 2-feature reweighter fits MC→data on
(zenith_pred, azimuth_pred) only, so any energy-dependent observable
(qtot, qmax, n_hits, ...) is left misaligned. This variant adds an
energy-proxy feature (log10 of per-event total Cherenkov charge) and
writes a separate weight CSV. Compare the two with the validation
plotting scripts to see how much of the residual qmax mismatch is just
an energy-spectrum problem versus genuine detector-response disagreement.

Caveat: with log_qtot in the feature set, the GB reweighter trivially
flattens the qtot marginal disagreement by construction. The interesting
diagnostic is what happens to qmax — if qmax also improves a lot, the
mismatch was driven by the energy spectrum (which we hadn't reweighted
on); if qmax barely budges, it's a per-pulse PMT/SPE-template / pulse-
extraction issue conditional on energy.

Outputs (per class):
    GB_and_base_weights_stopped_with_energy.csv
    GB_and_base_weights_through_with_energy.csv
    plots/roc_mc_vs_data_before_after_with_energy.png
"""
...
FEATURES = ["zenith_pred", "azimuth_pred", "log_qtot"]
SCORE_CUT = 0.5
SEED = 42

# GBReweighter hyperparameters (same as original; consider tightening
# min_samples_leaf when going from 2D → 3D to avoid overfitting)
GB_KWARGS = dict(
    n_estimators=80,
    learning_rate=0.1,
    max_depth=3,
...
def compute_event_qtot(source: str) -> pd.DataFrame:
    """Per-event qtot = sum(charge) over the merged pulsemap, both classes.

    Cached at cache/event_qtot_{source}.parquet — recompute by deleting
    that file. Pulls only event_no + charge from the per-class parquet
    files (column-pruned reads are seconds, not minutes).
    """
    cache = CACHE_DIR / f"event_qtot_{source}.parquet"
    if cache.exists():
        print(f"  qtot cache hit ← {cache.name}", flush=True)
        return pd.read_parquet(cache)

    parts = []
    for cls in ("stopped", "through"):
        path = PARQUET_DIR / f"{source}_{PULSEMAP}_{cls}.parquet"
        t0 = time.time()
        print(f"  reading {path.name} (event_no, charge) ...", flush=True)
        df = pd.read_parquet(path, columns=["event_no", "charge"])
        qtot = df.groupby("event_no", sort=False)["charge"].sum()
        parts.append(qtot)
        print(f"    {len(qtot):,} events  [{time.time()-t0:.0f}s]",
              flush=True)

    qtot_all = pd.concat(parts).rename("qtot").reset_index()
    qtot_all["log_qtot"] = np.log10(np.clip(qtot_all["qtot"].values,
                                            1e-3, None))
    qtot_all.to_parquet(cache)
    print(f"  cached → {cache}", flush=True)
    return qtot_all[["event_no", "qtot", "log_qtot"]]
...
def load_merged(source: str) -> pd.DataFrame:
    ...
    print(f"  computing per-event qtot for {source} ...", flush=True)
    qtot = compute_event_qtot(source)

    df = stopped.merge(zenaz, on="event_no", how="inner")
    df = df.merge(base, on="event_no", how="inner")
    df = df.merge(qtot, on="event_no", how="inner")
    df = df.dropna(subset=FEATURES + ["base_weight", "stopped_score"])
    df = df[df["base_weight"] > 0].reset_index(drop=True)
    df = df[np.isfinite(df["log_qtot"].values)].reset_index(drop=True)
    df["source"] = source
    print(f"  {source}: {len(df):,} events after merge/clean", flush=True)
    return df
...
def process_class(mc_all: pd.DataFrame, data_all: pd.DataFrame,
    ...
    print(f"  log_qtot — MC: median={mc['log_qtot'].median():.3f}, "
          f"data: median={data['log_qtot'].median():.3f}")
...
def plot_roc(roc_stopped: dict, roc_through: dict, out_path: Path) -> None:
    ...
    fig.suptitle(
        ...
        "(zenith_pred, azimuth_pred, log_qtot)",
        fontsize=13,
    )
    footer = (
        f"MC: {MC_DB.name}   |   "
...
def main():
    ...
    for stopped_flag, fname in [
        (True, "GB_and_base_weights_stopped_with_energy.csv"),
        (False, "GB_and_base_weights_through_with_energy.csv"),
    ]:
    ...
    plot_roc(
        ...
        plots_dir / "roc_mc_vs_data_before_after_with_energy.png",
    )
