...
Outputs (PREFIX = 'dynedge_muongun_neutrino'):
  - dynedge_muongun_neutrino_results.csv
  - dynedge_muongun_neutrino_metrics.json
  - dynedge_muongun_neutrino_state_dict.pth
  - dynedge_muongun_neutrino_model_config.yml
  - dynedge_muongun_neutrino_split.json
  - dynedge_muongun_neutrino_train_config.json
"""
...
def main() -> None:
    ...
    with sqlite3.connect(DB_PATH) as con:
        event_ids = pd.read_sql_query(
            f"SELECT event_no FROM truth WHERE pid IN (14, -14) AND energy >= 250 LIMIT {n_fetch}", con,
