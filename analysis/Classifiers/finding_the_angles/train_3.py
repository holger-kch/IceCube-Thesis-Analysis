#!/usr/bin/env python3
"""Improved DynEdge training for neutrino direction reconstruction.

Improvements over 2.ipynb:
- Proper train/val/test split (80/10/10)
- More events (5000)
- More features (+ rde, pmt_area if available)
- Better DynEdge config: 12 neighbours, explicit layer sizes, LayerNorm
- 20 epochs with early stopping (patience 5)
- Larger batch size (64), gradient clipping

Outputs (PREFIX = '3_direction'):
  - 3_direction_results.csv
  - 3_direction_metrics.json
  - 3_direction_state_dict.pth
  - 3_direction_model_config.yml
  - 3_direction_split.json
  - 3_direction_train_config.json
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train DynEdge for neutrino direction (v3).")
