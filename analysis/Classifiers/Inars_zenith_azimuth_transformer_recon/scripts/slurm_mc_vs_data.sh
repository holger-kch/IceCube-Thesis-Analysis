...
# Use the user's Python 3.9 runtime with local site-packages so torch, pandas,
# and pytz all resolve consistently on compute nodes.
PYTHON_EXEC="/usr/bin/python3.9"
export PATH="/groups/icecube/holgerkc/.local/bin:${PATH}"
export PYTHONPATH="/groups/icecube/holgerkc/.local/lib/python3.9/site-packages:${PYTHONPATH:-}"

echo "Using Python: $(${PYTHON_EXEC} -c 'import sys; print(sys.executable)')"
$PYTHON_EXEC "${PROJECT}/scripts/train_mc_vs_data.py" \
    --class-name "${CLASS}" \
    --pulsemap   "${PULSEMAP}" \
    ${UNIFY} \
