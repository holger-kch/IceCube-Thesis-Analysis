...
# Step 2: Multi-Task Transformer inference on BS muons (PID-selected from burnsample)
# Requires: data/bs_muon_event_nos.csv from step1
# Usage: sbatch slurm_step2_bs.sh
...
echo "=== Step 2: Reconstruct BS muon events with Multi-Task Transformer ==="
echo "Node: $(hostname) | GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null) | Date: $(date)"

python "${SCRIPT_DIR}/step2_reconstruct_events.py" --source bs --batch-size 256 --num-workers 4
